...
async function fetchMeteorologia(districtId, districtName) {
    ...
    for (const item of dataForecast.data) {
        ...
        <tr class="tabela">
            ...
            <td><img src="../images/${imageSrc}" height="55" alt="Weather Image"></td>
            <td>Min Temperature: ${item.tMin}°C</td>
            <td>Max Temperature: ${item.tMax}°C</td>
        </tr>`;
    }
    meteoHTML += '</table>';
    document.getElementById('previsao').innerHTML += `<h5>${districtName}</h5>${meteoHTML}`;
}
...
async function obterPrevisao() {
    ...
    try {
        ...
        * Monsanto pertence ao distrito de Castelo Branco. */
        ...
        if (['Almeida', 'Castelo Rodrigo', 'Linhares da Beira', 'Marialva'].includes(titleId)) {
            forecast = dataLocalDistrict.find(district => district.local === 'Guarda');
        } else if (titleId === 'Monsanto') {
            forecast = dataLocalDistrict.find(district => district.local === 'Castelo Branco');
        }
        ...
        if (forecast) {
            ...
            document.getElementById('previsao').innerHTML += '<p class="text-danger">Local não encontrado.</p>';
        }
    } catch (error) {
        console.log(error);
